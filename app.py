...
class UsageService:
    ...
    def _merge_claude_statusline_windows(self, api_windows: list[Window], statusline_windows: list[Window]) -> list[Window]:
        """Prefer Claude Code statusline primary limits over API aggregate limits.

        The OAuth usage API can report broad account/subscription windows that do
        not match the active Claude Code model/session shown to the user. The
        statusline file is generated by Claude Code itself, so it is the better
        source for the primary 5h and weekly dashboard bars. Keep API-only split
        windows such as weekly-sonnet/weekly-opus as secondary detail.
        """
        primary_overrides = {
            window.label: window
            for window in statusline_windows
            if window.label in {"session", "weekly"} and self._window_reset_is_current(window)
        }
        if not primary_overrides:
            return api_windows
        merged: list[Window] = []
        seen: set[str] = set()
        for window in api_windows:
            replacement = primary_overrides.get(window.label)
            if replacement is not None:
                merged.append(replacement)
                seen.add(window.label)
            else:
                merged.append(window)
        for label, window in primary_overrides.items():
