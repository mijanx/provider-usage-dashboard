...
class UsageService:
    ...
    def probe_claude(self) -> ProviderResult:
        ...
        if not windows:
            fallback_windows = self._claude_windows_from_statusline_file()
            if fallback_windows:
                ...
                return ProviderResult(provider="anthropic", status="ok", source="claude-statusline", windows=fallback_windows, plan=str(plan).upper() if plan else None, email=email)
        if not windows:
